...
#define SENTINEL_FILENAME "InOtIfY_sEnTiNeL"
#define SENTINEL_FILENAME_LEN (sizeof(SENTINEL_FILENAME)/sizeof(SENTINEL_FILENAME[0]))
...
struct watchent {
	...
	int sentinel_wait; /* set 1 while waiting for the sentinal to show up */
	int sentinel_seen; /* set 1 once sentinal seen */
};
...
static void remove_wd_from_tree(struct watchent *ent) {
	...
	tmp = tdelete(ent, &wd_info->tree_root, compare_wd_entries);
//	if (!tmp)
//		wd_info->tree_root = NULL;
}
...
//	printf("testing '%s' with '%s'\n", SENTINEL_FILENAME, path + strlen(path) - SENTINEL_FILENAME_LEN + 1);
	if (!strcmp(SENTINEL_FILENAME, path + strlen(path) - SENTINEL_FILENAME_LEN + 1)) {
printf("found sentinel?  %s\n", path);
		char *parent_dir;
		int ret;
//		tmp = path;

//		unlinkat(dir_fd, SENTINEL_FILENAME, AT_REMOVEDIR);
...
ret = mkdir(path, 0755);
return -ret;
				unlinkat(  , SENTINEL_FILENAME, AT_REMOVEDIR
*/
		ret = unlink(path);
		if (ret == -1)
...
printf("added watch for '%s'\n", path);
	...
	if (!new_watchent->sentinel_wait) {
//		printf("creating sentinel dir: %s/%s\n", path, SENTINEL_FILENAME);
		mkdirat(dir_fd, SENTINEL_FILENAME, 0000); /* trigger a dumb event */
		new_watchent->sentinel_wait++;
	}
//	if (!new_watchent->sentinel_seen)
//		return; /* still waiting for the sentinel...  wait to recurse */
	...
	for (;;) {
		...
		while (bpos < buf + nread) {
			...
			if ((!strcmp(temp_de->d_name, ".")) || (!strcmp(temp_de->d_name, "..")))
				continue;
			if (!strcmp(temp_de->d_name, SENTINEL_FILENAME)) {
				printf("found sentinel directory inside %s...updating times\n", path);
				struct timespec t[2] = {{0, UTIME_NOW}, {0, UTIME_NOW}};
				utimensat(dir_fd, SENTINEL_FILENAME, t, 0);
				continue;
			}
...
//			printf("looking for watchent for wd=%d\n", event->wd);
//			output("looking for watchent for wd=%d\n", event->wd);
//			twalk(wd_info->tree_root, dump_watch_tree);
...
//			if (event->mask & !IN_ISDIR && event->mask & IN_ACCESS && !strcmp(event->name, SENTINEL_FILENAME)) {
//			if (event->mask & !IN_ISDIR && event->mask & IN_ACCESS && !strcmp(event->name, SENTINEL_FILENAME)) {
//			if (event_compare(event->mask, IN_CREATE|IN_ISDIR) && !strcmp(event->name, SENTINEL_FILENAME)) {
//			if (event_compare(event->mask, IN_ISDIR) && !strcmp(event->name, SENTINEL_FILENAME)) {
//
//			if (event_compare(event->mask, IN_ISDIR) && (event->len == 0) && (!strcmp(wd_event_path + strlen(wd_event_path) - sizeof(SENTINEL_FILENAME), SENTINEL_FILENAME) - 1)) {
//			if (event_compare(event->mask, IN_ISDIR) && (event->len == 0) && (!strcmp(event_path + strlen(event_path) - sizeof(SENTINEL_FILENAME), SENTINEL_FILENAME) - 1)) {
printf("comparing '%s' and '%s', event_len=%d\n", event_path + strlen(event_path) - sizeof(SENTINEL_FILENAME) + 1, SENTINEL_FILENAME, event->len);
//			if (event_compare(event->mask, IN_ISDIR) && (event->len == 0) && (!strcmp(event_path + strlen(event_path) - sizeof(SENTINEL_FILENAME) + 1, SENTINEL_FILENAME))) {
			if (event_compare(event->mask, IN_ISDIR) && (!strcmp(event_path + strlen(event_path) - sizeof(SENTINEL_FILENAME) + 1, SENTINEL_FILENAME))) {
printf("event for the sentinel: wd=%d, name=%s, event_path=%s, sentinel_seen=%d\n", event->wd, event->len ? event->name : "NONE", event_path, tree_watchent->sentinel_seen);

				if (tree_watchent->sentinel_seen)
					goto next_event_freepath;

				int dir_fd;

//				tree_watchent = find_watchent_by_path(event->path);
//				printf("wd=%d, got sentinel filename for directory '%s'\n", event->wd, tree_watchent->path);
				tree_watchent->sentinel_seen ++;
				if ((dir_fd = open(tree_watchent->path, O_RDONLY|O_DIRECTORY)) < 0) { /* directory disappeared? */
//					ignore_watchent(tree_watchent);
					ignore_watchent(tree_watchent->wd);
//					struct watchent *keep_ptr = tree_watchent;
//					ret = tdelete(tree_watchent, &wd_info->tree_root, compare_wd_entries);
//					if (!ret)
//						wd_info->tree_root = NULL;
//					ret = tdelete(tree_watchent, &wd_info->

					printf("directory '%s' possibly disappeared: %m\n", tree_watchent->path);

				} else {
					unlinkat(dir_fd, SENTINEL_FILENAME, AT_REMOVEDIR);
					close(dir_fd);
					add_watches_recursive(wd_event_path, ENTRY_ADDED_EVENT);
				}

				goto next_event_freepath;
			} else if (event_compare(event->mask, IN_ISDIR)) {
				if (event_compare(event->mask, IN_CLOSE_NOWRITE) || event_compare(event->mask, IN_OPEN)) {
				}
			...
			/* check for some combinations we just don't care about */
//			for (i = 0 ; i < dontcare_masks_count ; i++) {
//				if (event->mask == dontcare_masks[i])
//					goto next_event;
//			}
...
//				tree_watchent = *(struct watchent **)tree_watchent;
//			} else {
//				output("did not find a matching watchent for event: %s\n", event_string);
//				free(event_string);
//				goto next_event;
//			}

//			output("result: wd=%d, path=%s\n", tree_watchent->wd, tree_watchent->path);
//			output("path: %s\n", tree_watchent->path);
...
#if 0
if (event->len) {
output("\twatch descriptor's path: '%s', event->name: '%s'\n", wd_event_path, event->name);
} else {
output("\twatch descriptor's path: '%s'\n", wd_event_path);
}
#endif
