...
//or sk_sk
		...
		switch (svsk->sk_sk->sk_family) {
			case AF_LOCAL:
				{
//                              const struct sockaddr_un *loc_sun = (struct sockaddr_un *)&svsk->sk_xprt.xpt_local;
//                              const struct sockaddr_un *rmt_sun = (struct sockaddr_un *)&svsk->xpt_remote;
//                              snprintf(buf, sizeof(buf), "local:%s", loc_sun->sun_path);
				snprintf(buf, sizeof(buf), "local:%s", ((struct rpc_xprt *)svsk)->address_strings[RPC_DISPLAY_ADDR]);
				break;
				}
			case AF_INET:
				{
