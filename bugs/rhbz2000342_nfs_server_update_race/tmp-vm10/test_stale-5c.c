...
int read_worker(void) {
	...
	while (42) {
		...
		if (current_run_state == run_state_pause) {
			...
			read_count = pread(fd, buf, BUF_SIZE, 0);
			close(fd);
...
//	write_file_byte(config->local_dfd, config->local_dir, STALE_DATA_FILE, buf, chars[char_num], true);
	...
	write_fd = openat(config->local_dfd, STALE_DATA_FILE, O_RDWR|O_DSYNC|O_TRUNC);
...
//			memset(buf, chars[char_num], BUF_SIZE);
			...
			write_fd = openat(config->local_dfd, STALE_DATA_FILE ".tmp", O_RDWR|O_SYNC|O_CREAT);
#else
			write_fd = openat(config->local_dfd, STALE_DATA_FILE, O_RDWR|O_DSYNC|O_TRUNC|O_CREAT);
#endif
...
//			fsync(write_fd);
			close(write_fd);
			renameat(config->local_dfd, STALE_DATA_FILE ".tmp", config->local_dfd, STALE_DATA_FILE);
#else
			close(write_fd);
#endif
