...
int run_mode_split_read(void) {
	char *buf = split_config->buf, this_char, actual_char1, actual_char2;
...
restart_loop:
		...
		actual_char1 = read_file_byte(split_config->dfd, split_config->basedir, "testfile", buf, true);

		if (actual_char1 == '\0')
			goto restart_loop;

		this_char = read_file_byte(split_config->dfd, split_config->basedir, "testfile", buf, false);
		if (this_char == '\0')
			goto restart_loop;

		actual_char2 = read_file_byte(split_config->dfd, split_config->basedir, "testfile", buf, true);
...
int run_mode_split_write(void) {
	...
	while (42) {
		...
		memset(buf, chars[char_num], LOCK_LEN);
		pwrite(split_config->control_fd, buf, BUF_SIZE, 0);
		output("%c", chars[char_num]);

		write_file_byte(split_config->dfd, split_config->basedir, "testfile", buf, chars[char_num], true);
...
#if KEEP_FD
	int fd = openat(single_config->local_dfd, "testfile", O_RDWR|O_DIRECT|O_SYNC);
#endif
	while (42) {
