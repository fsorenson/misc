...
#define FILE_BLOCKS (0UL) /* if we want to fallocate() 'contents' on the created files */
...
#define REMOVE_MOUNT_CONTENTS	0 /* whether to remove contents of the mountpoint to start over or prior to unmount */
#if REMOVE_MOUNT_CONTENTS	/* probably irrelevant -- we'll just recreate the filesystem each time instead */
#include <dirent.h>
...
char *make_dirname(uint64_t i) {
	if (! run_data.dirname)
		asprintf(&run_data.dirname, DIR_FMT, run_data.agcount + 1UL);
...
struct size_strs {
	uint64_t size;
	const char *str;
} size_strs[] = { /* could go overkill, and make them all really large, but let's not */
	{ (38*PiB) + 1, "error" }, /* can't reproduce with approximately this size or larger */
	{  17*PiB, "-dsunit=2048,swidth=2048" },
	{   6*PiB, "-dsunit=1024,swidth=1024" },
	{   1*PiB, "-dsunit=512,swidth=512" },
	{   1*TiB, "-dsunit=256,swidth=512" },
	{ XFS_SIZE_HOLE_MAX + 1, "-dsunit=256,swidth=256" },
	{ XFS_SIZE_HOLE_MIN, NULL }, /* build the string on the fly */
	{  MIN_IMAGE_SIZE, "-dsunit=256,swidth=256" },
	{   0, NULL },
};
char *get_sunit_swidth_str(void) {
	int i;

	if (run_data.image_size >= size_strs[0].size) {
		output("image size too large; maximum size: %" PRIu64 "\n", size_strs[0].size);
		exit(EXIT_FAILURE);
	}
...
//	return "-dsunit=256,swidth=256";
	return strdup("-dsunit=2048,swidth=2048");
	for (i = 1 ; i < ARRAY_SIZE(size_strs) ; i++) {
		if (run_data.image_size < size_strs[i].size)
			continue;
		if (size_strs[i].size == XFS_SIZE_HOLE_MIN) {
			char *str;
			asprintf(&str, "-dsize=%" PRIu64 ",sunit=256,swidth=256", XFS_SIZE_HOLE_MIN - 1);
			return str;
		}
		return size_strs[i].str;
	}
	output("image size too small; minimum size: %" PRIu64 "\n", size_strs[i-1].size);
	exit(EXIT_FAILURE);
}

int mkfs(const char *device) {
	char *sunit_swidth_str = get_sunit_swidth_str();
	int ret = exec_cmd(VERBOSITY_INFO, "/usr/sbin/mkfs.xfs", "-f" MKFS_XFS_ARGS, sunit_swidth_str, (char *)device);
...
#if REMOVE_MOUNT_CONTENTS
void remove_directory_contents(int dfd) {
	int nread;
	char *buf, *bpos;
	int de_count = 0;

	buf = malloc(GETDENTS_BUFSIZE);
	while ((nread = getdents64(dfd, buf, GETDENTS_BUFSIZE)) > 0) {
		bpos = buf;
		while (bpos < buf + nread) {
			...
			if (++de_count > 2) {
				if (de->d_type == DT_DIR) {
					int newdfd;
					if ((newdfd = openat(dfd, de->d_name, O_RDONLY|O_DIRECTORY)) < 0)
						output("could not open directory '%s': %m\n", de->d_name);
					else {
						remove_directory_contents(newdfd);
						close(newdfd);
					}

				}
				if ((unlinkat(dfd, de->d_name, (de->d_type == DT_DIR) ? AT_REMOVEDIR : 0)) < 0) {
					output("could not remove dir '%s': %m\n", de->d_name);
					exit(EXIT_FAILURE);
				}
			}
			bpos += de->d_reclen;
...
void remove_mount_contents(const char *mountpoint) {
	int dfd;

	if ((dfd = openat(AT_FDCWD, mountpoint, O_RDONLY|O_DIRECTORY)) < 0) {
		output("could not open mountpoint directory '%s': %m\n", mountpoint);
		return;
	}
	remove_directory_contents(dfd);
	close(dfd);
}
#endif
...
#if REMOVE_MOUNT_CONTENTS
			remove_mount_contents(mountpoint);
#endif
...
int main(int argc, char *argv[]) {
	...
	if (run_data.image_size < MIN_IMAGE_SIZE) {
		output("image size (%" PRIu64 ") is too small; must be at least 16 MiB (%" PRIu64 ")\n",
