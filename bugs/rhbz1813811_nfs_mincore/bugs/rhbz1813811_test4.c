...
int main(int argc, char *argv[]) {
	...
	char *fd1_map1 = NULL, *fd1_map2 = NULL, *fd1_remap, *fd2_map = NULL, *fd2_remap = NULL, *fd3_map, *path = NULL;
	...
	printf(" | %-*s", mincore_check_len, "open/map/close fd");
	printf("\n");
...
//			madvise(fd1_map1, test_size, MADV_WILLNEED);
			...
			if (use_locking_remap) {
				if ((fd1_remap = mmap(NULL, test_size, PROT_READ, MAP_SHARED, fd1, 0)) == MAP_FAILED)
					out_fail("unabale to re-mmap(fd1...): %m\n");
				printf(" | ");
