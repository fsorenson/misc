...
#define show_id(ugid) do { \
	...
	output("\t%" UG_DESC_WIDTH "s: %" UG_WIDTH "u -> '%s'\n", #ugid, ugid, \
	...
	if (getpwuid_r(uid, &pwdbuf, buf, pwsize, &pwd) != 0) {
		if (errno == ERANGE) {
			...
			if ((tmp = realloc(buf, pwsize)) == NULL) {
				output("error reallocating: %m\n");
				exit(EXIT_FAILURE);
			}
			buf = tmp;
			goto retry;
		} else {
			output("error getting passwd: %m\n");
			exit(EXIT_FAILURE);
		}
	}
	if (pwd == NULL)
...
void show_uid_gids(void) {
	output("uid/gid of account:\n");
	...
	if ((grp_count = getgroups(0, NULL)) > 0) {
		...
		if ((ret = getgroups(grp_count, groups)) > 0) {
			if (ret == grp_count) {
				...
				output("error occurred while getting groups: %m\n");
		}
		free(groups);
	} else if (grp_count == 0)
		output("\tuser is not a member of any groups?\n");
	else
		output("error occurred while getting groups: %m\n");
}
