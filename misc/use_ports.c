...
int main(int argc, char *argv[]) {
	...
	if (argc == 1) {
		fill_open_range(&to_open, MIN_PORT, MAX_PORT);
		to_open_count = MAX_PORT - MIN_PORT + 1;
	} else if (argc == 2) {
		int low, high;
		char *p;

		low = strtol(argv[1], &p, 10);
		if (low == 0) {
		...
		to_open_count = high - low + 1;
	} else {
		to_open = malloc((argc - 1) * sizeof(int));
